...
在现有 AI 摘要窗口里新增一个 tab：`人物关系`。这是 AI 摘要窗口内部的 tab，和现有 `正文 / 设置` 同级，不新增独立页面，也不改 AI资料页入口。它不是完整 AI资料页，也不是人物列表；它只回答读者阅读当前章节时最需要的问题：主角现在和谁有关，这些关系最近是什么状态。

## MVP 范围

采用「图 + 说明列表」方案。设计读法：这是小说阅读器里的内嵌阅读辅助，视觉要安静、轻、精致，贴近现有 Reader 摘要卡片，不做数据驾驶舱。

- 图：主角居中，周围展示与主角关系最强/最近的角色。
- 线：按关系类型着色，同一角色与主角有多种关系时聚合成一条线。
- 列表：补充图上放不下的语义，例如关系类型、最近动态、关系摘要。
- 视觉：优先使用 HTML 节点 + SVG/Canvas 连接层的混合方案；SVG 只负责连线，不承担全部视觉表达。若纯 SVG 效果粗糙，可以改为 CSS 节点、渐变边、柔和阴影和轻量 motion。
...
在 AI 摘要窗口 tab 区新增：`人物关系`，与 `正文`、`设置` 同级。加完 tab 后必须确认 inline、continuous、side 三种摘要面板都显示一致，并且默认仍停留在 `正文` tab。
...
3. 视觉质量要求
   - 不是裸 SVG 圆点图。节点要有层级、留白、柔和边界和当前 Reader 风格一致的暖色主 accent。
   - 图上只放短标签，长解释进列表，避免拥挤。
   - 颜色克制，最多一种主 accent 加关系类型的低饱和辅助色。
   - 节点和线条要支持亮/暗主题，contrast 不低于现有摘要卡片。
   - loading / empty / error 状态要和最终图形轮廓一致，不用突兀 spinner。
...
- AI 摘要窗口新增 `人物关系` tab。
- 用 HTML/CSS 节点 + SVG 或 Canvas 连接层 + HTML 列表渲染；默认不引入图表库。
- inline / continuous / side 三处摘要面板都要有同一个 tab。

不做：

- 后端改动
- 图布局库，除非手写布局在实际 UI 验收里明显不够美观
- 拖拽/缩放
- 点击展开详情
- 全人物关系图
- AI 重新生成文案
...
- 如果图做得像工程调试图，即使数据正确也不算完成；需要按 taste 要求做 UI 审核。
